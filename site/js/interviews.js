...
const homeHtml = (item) => {
  ...
  return `
    ...
    <article class="media-feature">
      <a class="video-frame media-thumb" href="${escapeHtml(item.href)}">
        ${thumbImg(item)}
        <span class="media-play" aria-hidden="true">${playIcon}</span>
        <span class="visually-hidden">Ver entrevista</span>
      </a>
...
const isPlaceholder = (value) => /pendiente/i.test(String(value || "").trim());

const usableHighlights = (item) =>
  (item.highlights || []).filter((point) => String(point || "").trim() && !isPlaceholder(point));

const usableContext = (item) => {
  const text = String(item?.context || "").trim();
...
const pageHtml = (item) => {
  ...
  const facts = [
    ...
    ["Cargo", item.role],
    ["Medio", item.media],
    ["Duración", item.duration && !isPlaceholder(item.duration) ? item.duration : ""],
  ]
    .filter(([, value]) => value)
    .map(([label, value]) => `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(value)}</dd></div>`)
    .join("");
  const points = usableHighlights(item)
    .map((point) => `<li>${escapeHtml(point)}</li>`)
    .join("");
  const context = usableContext(item);
  return `
...
const relatedHtml = (post) => {
  const image = window.CavedrepaCovers?.src(post) || "/images/home/tractor.jpg";
  ...
  return `
    <a class="news-card" href="${escapeHtml(href)}">
      <img src="${escapeHtml(image)}" alt="" width="640" height="400" onerror="this.src='/images/home/tractor.jpg'">
      <div class="news-body">
